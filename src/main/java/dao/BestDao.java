...
public class BestDao {
	...
	public List<Map<String,Object>> selectBestListByPage(){
		...
		String sql = " SELECT p.product_no, p.name, p.price,p.picture ,p.volume,p.alcohol_level, o.product_no, o.cnt, @rownum:=@rownum+1 AS ranking "
				+ " FROM (SELECT product_no,COUNT(*) cnt,@rownum:=0 FROM `order` GROUP BY product_no) o INNER JOIN product p "
